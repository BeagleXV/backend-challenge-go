...
- `httpapi` (Fase 6, com `/health/ready` estendido na Fase 9 para checar SQS além de Postgres) — provê o `http.Handler` (`httpapi.NewRouter`, dependendo de todo o resto: verifier, pool, `*sqs.Client`, os quatro serviços de aplicação e os `ports.*Repository`) e registra `OnStart`/`OnStop` do `*http.Server`: `OnStart` faz `net.Listen` (erro de bind falha o processo imediatamente) e sobe `srv.Serve` em goroutine; `OnStop` chama `srv.Shutdown(ctx)` — para de aceitar conexões novas e espera as em andamento terminarem, respeitando o `ctx` que `fx.StopTimeout` limita.
- `bootstrap` — desde a Fase 6, o próprio `fx.Invoke` de `httpapi` já força a construção de `config`/`logging`/`postgres`/`idp`/`application`/`sqsconsumer` (o router depende de tudo isso, inclusive do `*sqs.Client`). `bootstrap` ficou reduzido a um `fx.Invoke(func(*resolvependingreference.Service) {})`, só para alcançar esse serviço específico, que nenhum outro componente ainda usa (fica para o worker de referências pendentes, Fase 10). Quando esse worker existir, este invoke também deve ser removido.

**Ordem de registro dos módulos importa para a ordem de shutdown:** `SQSConsumerModule` é listado *antes* de `HTTPAPIModule` em `All()` propositalmente — como `fx` desfaz `OnStop` na ordem inversa em que os `OnStart` rodaram (LIFO), listar o consumidor SQS primeiro garante que seu `OnStart` rode primeiro e, portanto, seu `OnStop` rode **depois** do HTTP. Ordem de shutdown obtida, confirmada nos logs de uma execução real (`docker compose up` + `SIGTERM`, ver seção 10): servidor HTTP → consumidor SQS → pool Postgres → sync do logger — exatamente a ordem que o desafio pede (listener HTTP para de aceitar requisições antes de qualquer worker parar, que por sua vez para antes de qualquer conexão de infraestrutura fechar). O worker de outbox (Fase 11) e o de referências pendentes (Fase 10) entram nessa cadeia do mesmo jeito: listados antes do `httpapi`, para que seus `OnStop` também rodem antes do dele.
...
## 13. Limitações e trabalho não concluído
