> **Nota:** este é o enunciado original do desafio, preservado sem alterações para referência (é a fonte de expressões citadas ao longo deste repositório, como "seção 13 do README"). A documentação da solução implementada está em [`README.md`](../README.md); as decisões de arquitetura, em [`ARCHITECTURE.md`](ARCHITECTURE.md).
